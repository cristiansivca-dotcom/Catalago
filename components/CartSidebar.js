...
const CartSidebar = ({ isOpen, onClose, cart, removeFromCart, onQuote }) => (
  <>
    ...
    {/* Sidebar panel */}
    <div
      className={`fixed inset-y-0 right-0 w-80 bg-white shadow-2xl transform transition-transform duration-300 z-50 ${isOpen ? "translate-x-0" : "translate-x-full"}`}
    >
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-1">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div
                key={item.id}
                className="group flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-transparent hover:border-blue-100 hover:bg-white hover:shadow-sm transition-all"
              >
                <div className="w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center overflow-hidden border border-gray-100 relative">
                  {item.fotos && item.fotos.length > 0 ? (
                    <Image src={item.fotos[0]} alt={item.nombre} fill className="object-cover" />
                  ) : (
